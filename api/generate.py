...
def _upload_to_blob(zip_bytes: bytes, filename: str) -> str:
    ...
    # access=public is a required query param so the URL is browser-downloadable
    import urllib.parse
    api_url = f"https://blob.vercel-storage.com/{urllib.parse.quote(filename)}?access=public"
    ...
    try:
        ...
        raise RuntimeError(
            f"Vercel Blob API returned {e.code} {e.reason}. "
            f"URL: {api_url}. "
